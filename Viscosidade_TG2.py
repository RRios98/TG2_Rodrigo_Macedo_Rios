...
class DynamicPlotApp:
    ...
    def gerar_marc_temp(self):
        ...
        if self.passo == 0:
            ...
            self.error_label.config(text=f"O intervalo de {self.temp_tot} min foi dividido em {math.floor(self.temp_tot / self.passo) + 1} marcações. A primeira marcação é de {self.tempo_medidas[0]} min e a última é {(self.temp_tot / self.passo)*self.passo} min.")
            ...
            if len(self.tempo_medidas) == 0:
                ...
                if valor_laboratorio < 0:
                    ...
                    if (0.01*self.tempo_medidas[self.contador_lab] + 10.378) < math.log(valor_laboratorio) or (0.0082*self.tempo_medidas[self.contador_lab] + 8.49) > math.log(valor_laboratorio):
                        self.error_label.config(text=f"O valor de viscosidade {valor_laboratorio} cP, que não satisfaz os critérios de aceite para a escala laboratorial, relativo à marcação temporal {self.tempo_medidas[self.contador_lab]} min foi coletada.")
                        self.insat_tempo_lab.append(self.tempo_medidas[self.contador_lab])
                        self.insat_valor_lab.append(valor_laboratorio)
                    else:
                        self.error_label.config(text=f"O valor de viscosidade {valor_laboratorio} cP para a escala laboratorial relativo à marcação temporal {self.tempo_medidas[self.contador_lab]} min foi coletada.")
            else:
            ...
            if len(self.tempo_medidas) == 0:
                ...
                if valor_processo < 0:
                    ...
                    if (0.01*self.tempo_medidas[self.contador_proc] + 10.378) < math.log(valor_processo) or (0.0082*self.tempo_medidas[self.contador_proc] + 8.49) > math.log(valor_processo):
                        self.error_label.config(text=f"O valor de viscosidade {valor_processo} cP, que não satisfaz os critérios de aceite para a escala processual, relativo à marcação temporal {self.tempo_medidas[self.contador_proc]} min foi coletada.")
                        self.insat_tempo_proc.append(self.tempo_medidas[self.contador_proc])
                        self.insat_valor_proc.append(valor_processo)
                    else:
                        self.error_label.config(text=f"O valor de viscosidade {valor_processo} cP para a escala processual relativo à marcação temporal {self.tempo_medidas[self.contador_proc]} min foi coletada.")
            else: 
